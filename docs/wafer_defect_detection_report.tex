...
%%% AI 570 -- Deep Learning | Spring 2026 | Team 4
%%% ==========================================================================
...
\IEEEauthorblockA{School of Graduate Professional Studies\\
...
study we evaluate three convolutional neural network (CNN)
architectures---a custom four-block CNN, ResNet-18, and EfficientNet-B0---on
the WM-811K dataset, which contains approximately 811,000 wafer maps spanning
nine defect classes. Because the dataset exhibits severe class imbalance (the
majority ``none'' class outnumbers the rarest class by nearly 1{,}000:1), we
employ weighted random sampling, class-weighted cross-entropy loss, and
standard data-augmentation strategies. Under a CPU-constrained training budget
of three epochs per model, these rebalancing techniques deliberately
sacrifice overall accuracy in favor of defect-class sensitivity: the custom CNN
achieves the highest macro-averaged F1 score (0.50) with defect-class recall
exceeding 85\% for several pattern types, while overall accuracy remains low
($\sim$10\%) because the dominant ``none'' class is effectively suppressed.
Gradient-weighted Class Activation Mapping (Grad-CAM) visualizations confirm
that the learned features correspond to spatially meaningful defect regions.
\end{abstract}
...
classification on the WM-811K dataset. Under CPU-constrained 3-epoch training,
the custom CNN achieved the highest macro F1 (0.501), with several defect
classes detected at high fidelity (Edge-Ring: 0.923, Center: 0.843,
Near-full: 0.733). Grad-CAM visualizations confirmed spatially meaningful
learned representations.

Future work includes: (1)~extended GPU training for 25+ epochs;
(2)~a two-stage classification pipeline to decouple
sensitivity--specificity; (3)~Vision Transformers for long-range spatial
dependencies; (4)~GAN-based augmentation~\cite{chawla2002} for
underrepresented classes; and (5)~mixed-type defect
detection~\cite{kim2022}.
