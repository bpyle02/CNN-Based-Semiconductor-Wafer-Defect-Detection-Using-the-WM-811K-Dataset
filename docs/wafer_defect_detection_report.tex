...
With only 5~epochs, models are far from convergence. The custom CNN improves
...
% NOTE: Macro F1 values will be updated with actual training results.
The custom CNN outperforms both pretrained models in macro~F1. This reversal
of the typical transfer-learning advantage~\cite{pan2010} is explained by the
five-epoch budget: frozen early layers restrict gradient flow in pretrained
models, while the custom CNN updates all parameters from initialization. With
longer training, pretrained features would likely surpass the custom
...
``none''-class prevalence. Under five epochs, overall accuracy remains low
because the models have not yet converged on the dominant class; however,
defect-class recall is high for several categories. In manufacturing, this
