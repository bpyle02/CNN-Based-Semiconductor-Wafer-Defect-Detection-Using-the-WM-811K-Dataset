...
\IEEEauthorblockA{The Pennsylvania State University\\
...
defect patterns to maintain high yield and reduce production cost. We present
a modular deep learning pipeline for multi-class wafer-defect classification
on the WM-811K dataset (${\sim}172{,}000$ labeled samples, 9~classes, 85\%
majority ``none'' class). We compare three architectures---a custom CNN,
fine-tuned ResNet-18, and fine-tuned EfficientNet-B0---trained with
class-weighted cross-entropy loss and natural distribution preservation.
Under CPU-constrained five-epoch training, the custom CNN achieves the
highest macro~F1, demonstrating competitive defect-class recognition despite
limited compute budget. Grad-CAM visualizations confirm spatially meaningful
learned representations aligned with known defect morphologies.
\end{abstract}
...
5~epochs of CPU training.

\begin{table}[H]
\centering
\caption{Test-set performance comparison (5~epochs, CPU). Best in \textbf{bold}.}
...
\midrule
Custom CNN       & \textbf{88.6} & 0.610 & \textbf{0.904} & 11{,}400 \\
ResNet-18        & 83.7 & \textbf{0.665} & 0.875 & 6{,}075 \\
EfficientNet-B0  & 86.0 & 0.608 & 0.887 & 3{,}630 \\
\bottomrule
\end{tabular}
\end{table}

The custom CNN achieves the highest overall accuracy (88.6\%) and weighted F1
(0.904), while ResNet-18 achieves the highest macro~F1 (0.665), indicating
stronger per-class balance across rare defect types. All three models
successfully learn the dominant ``none'' class (F1 $> 0.9$), validating the
...
\begin{table}[H]
\centering
\caption{Per-class metrics for the custom CNN (5~epochs).}
\label{tab:per_class}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Class} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1}
               & \textbf{Support} \\
\midrule
Center      & 0.841 & 0.846 & 0.844 & 644 \\
Donut       & 0.467 & 0.843 & 0.601 & 83 \\
Edge-Loc    & 0.234 & 0.592 & 0.336 & 779 \\
Edge-Ring   & 0.989 & 0.864 & 0.922 & 1{,}452 \\
Loc         & 0.331 & 0.458 & 0.384 & 539 \\
Near-full   & 0.600 & 0.955 & 0.737 & 22 \\
Random      & 0.513 & 0.915 & 0.658 & 130 \\
Scratch     & 0.058 & 0.067 & 0.062 & 179 \\
None        & 0.979 & 0.916 & 0.946 & 22{,}115 \\
\bottomrule
\end{tabular}
\end{table}

The ``none'' class achieves F1 of 0.946, confirming that the corrected
methodology (removing WeightedRandomSampler, preserving natural distribution)
enables proper learning of the dominant class. Edge-Ring achieves the highest
defect-class F1 (0.922) due to its distinctive annular morphology and large
support. Near-full achieves 95.5\% recall despite only 22 test samples.
Scratch remains the weakest class (F1 = 0.062), likely due to its thin linear
morphology and low support (179 samples).

\subsection{Training Dynamics}

Over 5~epochs, all models converge rapidly. The custom CNN improves from
81.7\% to 91.6\% validation accuracy (best at epoch~4); ResNet-18 from
83.1\% to 88.4\% (best at epoch~4); EfficientNet-B0 from 80.3\% to 86.3\%
(best at epoch~5). The continued improvement at epoch~5 suggests that
extended training could yield further gains, particularly for rare classes.
...
\textbf{ResNet-18 achieves highest macro~F1.}
While the custom CNN achieves the highest accuracy (88.6\%) and weighted~F1
(0.904), ResNet-18 attains the best macro~F1 (0.665), indicating stronger
per-class balance. This is explained by ResNet-18's pretrained ImageNet
features~\cite{pan2010,yosinski2014}, which---once properly normalized---help
the model generalize better on rare defect classes despite frozen early
layers. With longer training, the transfer-learning advantage would likely
widen further.

\textbf{The accuracy--sensitivity trade-off.}
By removing the WeightedRandomSampler and relying on class-weighted
cross-entropy loss, the training distribution matches the natural 85\%
``none''-class prevalence. All models achieve ``none''-class F1 above 0.9,
confirming that the corrected methodology allows the dominant class to be
learned alongside rare defects. In manufacturing, class-weighted loss
provides a principled mechanism to penalize missed defects while preserving
natural prevalence for calibrated predictions. A two-stage pipeline
(defect/no-defect, then defect typing) could further improve the operating
point.

\textbf{Five-epoch limitation.}
While all three models achieve 83--89\% accuracy and macro~F1 of 0.61--0.67,
continued validation improvement at epoch~5 suggests the models have not
fully converged. Extended GPU training (25+ epochs) with learning-rate
scheduling would likely improve rare-class performance (e.g., Scratch
F1 = 0.062) without degrading the dominant-class recognition already
achieved.
...
distribution (removing weighted sampling), applying correct ImageNet
normalization for pretrained models, and using layer-boundary freezing for
transfer learning, all three models achieved 83--89\% overall accuracy under
CPU-constrained five-epoch training. The custom CNN attained the highest
accuracy (88.6\%) and weighted~F1 (0.904), while ResNet-18 achieved the best
macro~F1 (0.665). Critically, the ``none'' class---which collapsed to
F1 = 0.000 under weighted sampling---now achieves F1 above 0.9 for all
models. Grad-CAM visualizations confirmed spatially meaningful learned
representations aligned with known defect morphologies.
...
distribution-distorting samplers; (2)~establishing that ImageNet normalization
must be applied consistently during both training and evaluation for
pretrained models; (3)~providing a reproducible training pipeline with
stratified validation for severely imbalanced industrial data.
