...
xp ∈RN×(P 2·C), where D is the feature dimension, P is the patch resolution, and N = HW/P 2
...
3
VITS AND MLP-MIXERS CONVERGE AT SHARP LOCAL MINIMA
...
parameters LN
train = Eϵ∼N [Ltrain(w +ϵ)] in Table 1, which reveals the average ﬂatness. Although
...
4
A PRINCIPLED OPTIMIZER FOR CONVOLUTION-FREE ARCHITECTURES
...
4.1
SAM: OVERVIEW
Intuitively, SAM (Foret et al., 2021) seeks to ﬁnd the parameter w whose entire neighbours have low
...
the exact solution of the inner maximization ϵ⋆= arg max∥ϵ∥2≤ρ Ltrain(w + ϵ) is hard to obtain,
they employ an efﬁcient ﬁrst-order approximation:
ˆϵ(w) = arg max
∥ϵ∥2≤ρ
Ltrain(w) + ϵT ∇wLtrain(w) = ρ∇wLtrain(w)/∥∇wLtrain(w)∥2.
(2)
...
4.2
SHARPNESS-AWARE OPTIMIZATION IMPROVES VITS AND MLP-MIXERS
...
4.3
VITS OUTPERFORM RESNETS WITHOUT PRE-TRAINING OR STRONG AUGMENTATIONS
...
Model
λmax of diagonal blocks of Hessian
∥w∥2
∥a1∥2
∥a6∥2
∥a12∥2
...
attempting to analyze what speciﬁc components cause the blowing up of λmax in the models trained
without SAM. We observe that shallower layers have larger Hessian eigenvalues λmax, and the ﬁrst
...
5.1
WHEN SCALING THE TRAINING SET SIZE
...
In Hal Daumé III and Aarti Singh (eds.),
...
tomated data augmentation with a reduced search space.
...
Mean ﬁeld residual networks: On the edge of chaos.
In
...
3072
B
WHEN SAM MEETS ADVERSARIAL TRAINING
...
max
ϵ∈Ssam max
δ∈Sadv Ltrain(w + ϵ, x + δ, y),
(5)
where Ssam and Sadv denote the allowed perturbation norm balls for the model parameter w and in-
put image x, respectively. Note that we can simultaneously obtain the gradients for computing ϵ and
...
adversarial robustness.
C
WHEN SAM MEETS CONTRASTIVE LEARNING
...
256
along with a cosine decay schedule. For the second
...
D
WHEN SAM MEETS TRANSFER LEARNING
...
respectively. Then we upsample A to the image shape H × W before generating the ﬁgure.
17
...
v3 cores (2 per chip), resulting in 32 images per core. The SAM computation for ˆϵ is conducted on
each core independently.
H.1
PERTURBATION STRENGTH IN SAM
...
2021), we also ﬁnd that a relative small ρ ∈[0.02, 0.05] works the best for ResNets. However, larger
...
learning already improve generalization, the suitable ρ becomes signiﬁcantly smaller. Note that we
do not re-tune any other hyperparameters when using SAM.
H.2
TRAINING ON IMAGENET SUBSETS
...
I
LONGER SCHEDULE OF VANILLA SGD
