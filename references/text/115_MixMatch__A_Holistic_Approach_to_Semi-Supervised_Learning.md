...
**Authors**: Berthelot, Carlini, Goodfellow, Papernot, Oliver, Raffel
**Year**: 2019
**arXiv**: 1905.02249
...
arXiv:1905.02249v2  [cs.LG]  23 Oct 2019
...
pair in ˆ
X, we compute MixUp( ˆ
Xi, Wi) and add the result to the collection X ′ (algorithm 1, line
13). We compute U′
i = MixUp( ˆUi, Wi+| ˆ
X|) for i ∈(1, . . . , | ˆU|), intentionally using the remainder
...
SVHN and SVHN+Extra
As with CIFAR-10, we evaluate the performance of each SSL method
...
STL-10
STL-10 contains 5000 training examples aimed at being used with 10 predeﬁned folds (we
...
A (one-hot) label
L
The number of possible label classes (the dimensionality of p)
X
A batch of labeled examples and their labels
X ′
A batch of processed labeled examples produced by MixMatch
u
...
Augment(x)
A stochastic data augmentation function that returns a modiﬁed version of x. For
...
λU
A hyper-parameter weighting the contribution of the unlabeled examples to the
training loss
α
Hyperparameter for the Beta distribution used in MixUp
T
...
MixMatch
Supervised
Figure 4: Error rate comparison of MixMatch to baseline methods on SVHN+Extra for a varying
number of labels. With 250 examples we reach nearly the state of the art compared to supervised
training for this model.
C
13-layer ConvNet results
Early work on semi-supervised learning used a 13-layer convolutional network architecture [31, 44,
25]. In table 8 we present results on a similar architecture. We caution against comparing these
numbers directly to previous work as we use a different implementation and training process [35].
Method
CIFAR-10
SVHN
250
