...
**Authors**: Yu, Li, Jiang, Yu, Shi, Wang
**Year**: 2022
**arXiv**: 2111.11418
**Topic**: transformer
...
weihaoyu6@gmail.com
{luomi,zhoupan,sicy,zhouyc,fengjs,yansc}@sea.com
xinchao@nus.edu.sg
...
achieves competitive performance on multiple computer vi-
sion tasks.
For example, on ImageNet-1K, PoolFormer
...
and 50%/62% fewer MACs.
The effectiveness of Pool-
...
architecture design.
arXiv:2111.11418v3  [cs.CV]  4 Jul 2022

--- Page 2 ---
1. Introduction
Transformers have gained much interest and success in
the computer vision field [3, 8, 44, 55].
Since the semi-
...
to the attention-based token mixer [56].
Based on this
...
to conduct only basic token mixing.
Astonishingly, this
...
[53] and ResMLP [52], as shown in Figure 1(b).
More
...
sults demonstrate that MetaFormer, even with a naive token
mixer, can still deliver promising performance. We thus ar-
...
the same as Transformers. The input I is first processed by
input embedding, such as patch embedding for ViTs [17],
  X = \mathrm {InputEmb}(I), 
(1)
where X ∈RN×C denotes the embedding tokens with se-
quence length N and embedding dimension C.
Then, embedding tokens are fed to repeated MetaFormer
blocks, each of which includes two residual sub-blocks.
Specifically, the first sub-block mainly contains a token
mixer to communicate information among tokens and this
sub-block can be expressed as
  Y = \mathrm {TokenM i xer}(\mathrm {Norm}(X)) + X, 
(2)
where
Norm(·)
denotes
...
layered MLP with non-linear activation,
  Z = \sig ma (\m a thrm {Norm}(Y)W_1)W_2 + Y, 
(3)
where W1 ∈RC×rC and W2 ∈RrC×C are learnable pa-
...
c
 {1}{K \times K} \sum _{p,q=1}^{K}T_{:, i+p-\frac {K+1}{2}, i+q-\frac {K+1}{2}} - T_{:, i, j}, (4)
where K is the pooling size. Since the MetaFormer block
...
(b) PoolFormer block
Figure 2. (a) The overall framework of PoolFormer. Similar to [24,36,57], PoolFormer adopts hierarchical architecture with 4 stages.
For a model with L PoolFormer blocks, stage [1, 2, 3, 4] have [L/6, L/6, L/2, L/6] blocks, respectively. The feature dimension Di of stage i
...
Former. Specifically, PoolFormer has 4 stages with H
4 × W
4 ,
H
8 × W
...
H and W represent the width and height of the input image.
...
suming there are L PoolFormer blocks in total, stages 1, 2,
3, and 4 will contain L/6, L/6, L/2, and L/6 PoolFormer
...
The models are trained for 300 epochs using AdamW opti-
Stage #Tokens
Layer Specification
PoolFormer
S12 S24 S36 M36 M48
...
mizer [29,37] with weight decay 0.05 and peak learning rate
lr = 1e−3 · batch size/1024 (batch size 4096 and learning
rate 4e−3 are used in this paper). The number of warmup
...
4.1
79.8
RSB-ResNet-101 [24,59]
224
45
7.9
81.3
RSB-ResNet-152 [24,59]
224
60
11.6
81.8
...
as shown in Section 4.4. See the appendix for more details
on hyper-parameters. Our implementation is based on the
...
--- Page 6 ---
like models.
For example, PoolFormer-S24 reaches the
top-1 accuracy of more than 80 while only requiring 21M
parameters and 3.4G MACs.
Comparatively, the well-
...
formance.
Specifically, the pyramid Transformer PVT-
...
Setup.
We evaluate PoolFormer on the challenging
...
with an initial learning rate of 1×10−4 and batch size of 16.
...
side of the images is also resized to 800 pixels. The imple-
mentation is based on the mmdetection [4] codebase and
the experiments are run on 8 NVIDIA A100 GPUs.
Results.
Equipped with RetinaNet for object detection,
...
learning rate of 2 × 10−4 that will decay in the polynomial
decay schedule with a power of 0.9. Images are resized
and cropped into 512 × 512 for training and are resized to
...
RetinaNet 1×
Mask R-CNN 1×
Params (M) AP AP50 AP75 APS APM APL Params (M) APb APb
50 APb
75 APm APm
50 APm
75
...
COCO val2017 [34]. 1× training schedule (i.e.12 epochs) is used for training detection models. AP b and AP m represent bounding
...
mantic FPN [30].
Token mixers.
Compared with Transformers, the main
change made by PoolFormer is using simple pooling as
a token mixer.
We first conduct ablation for this oper-
...
trix WR ∈RN×N for each block. The matrix is initial-
...
where X ∈RN×C are the input token features with the to-
ken length of N and channel dimension of C. The token
...
well even with random token mixing, not to say with other
...
1.8
77.2
Other components Residual connection [25] →None
11.9
1.8
0.1
Channel MLP →None
...
ablation study. The top-1 accuracy on the validation set is reported.
∗This token mixer utilizes global random matrix WR ∈RN×N
(parameters are frozen after random initialization) to conduct token mixing by X′ = WRX where X ∈RN×C are input tokens with
the token length of N and channel dimension of C.
†Modified Layer Normalization (MLN) computes the mean and variance along
...
1 in the top one or two stages in PoolFormer. From Ta-
...
token mixer, so we call it FC.
architecture termed MetaFormer where the token mixer is
...
for the support of partial computational resources.
This
project is in part supported by NUS Faculty Research Com-
mittee Grant (WBS: A-0009440-00-00).
Shuicheng Yan
...
Yan, and Jiashi Feng.
Aˆ 2-nets: Double attention net-
...
[18] Stefan Elfwing, Eiji Uchibe, and Kenji Doya.
Sigmoid-
...
Cmt:
Convolutional
neural networks meet vision transformers.
arXiv preprint
...
ference on Computer Vision, pages 6023–6032, 2019. 4
...
channel dimension in vanilla Layer Normalization.
The
...
M36
M48
Peak drop rate of stoch. depth dr
0.1
0.1
0.2
0.3
0.4
LayerScale initialization ϵ
10−5
10−5
10−6
10−6
