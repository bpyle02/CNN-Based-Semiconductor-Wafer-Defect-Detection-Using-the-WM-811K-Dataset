...
**Authors**: Rabanser, Gunnemann, Lipton
**Year**: 2019
**arXiv**: 1810.11953
**Topic**: deployment
...
detecting that p(f(x)) ̸= q(f(x)). Building on their idea of combining black-box dimensionality
...
1, ..., x′
m} ∼q, our task is
to determine whether p(x) equals q(x′). Formally, H0 : p(x) = q(x′) vs HA : p(x) ̸= q(x′).
...
): Principal components analysis is a standard tool that
...
mentioned constraints such that ˆ
X = XR yields a more compact data representation.
Sparse Random Projection (SRP
): Since computing the optimal transformation might be expen-
...
sionality of a given data matrix X, such that ˆ
X = XR. The elements of R are generated using the
...
Autoencoders (TAE
and UAE
): We compare the above-mentioned linear models to non-linear
...
to reduce the reconstruction loss: φ, ψ = arg minφ,ψ ∥X −(ψ ◦φ)X∥2.
...
σ ∥x−˜x∥2 and set σ to the median distance
...
j=1
Oij
Nsum being the fraction of row
totals, and p•j = O•j
Nsum = P2
i=1
Oij
Nsum being the fraction of column totals. The relevant test statistic
...
vs HA : acc ̸= 0.5. Under the null hypothesis, the accuracy of the classiﬁer follows a binomial
...
in High Dimensions. In Association for the Advancement of Artiﬁcial Intelligence (AAAI),
2015.
...
[46] R John Simes.
An Improved Bonferroni Procedure for Multiple Tests of Signiﬁcance.
Biometrika, 1986.
...
--- Page 13 ---
[53] Vladimir Vovk and Ruodu Wang.
...
--- Page 14 ---
A
Detailed Shift Detection Results
Our complete shift detection results in which we evaluate different kinds of target shifts on MNIST
and CIFAR-10 using the proposed methods are documented below. In addition to our artiﬁcially
...
(g) Top different samples.
(h) Top similar samples.
Figure 21: MNIST medium image shift (50%, ﬁxed) plus knock-out shift (variable), univariate two-
...
BBSDs
(c) Knock out 100% of class 0.
Figure 22: MNIST medium image shift (50%, ﬁxed) plus knock-out shift (variable), multivariate
two-sample tests.
22
...
(g) Top different samples.
(h) Top similar samples.
Figure 23: MNIST only-zero shift (ﬁxed) plus medium image shift (variable), univariate two-sample
...
BBSDs
(c) 100% perturbed samples.
Figure 24: MNIST only-zero shift (ﬁxed) plus medium image shift (variable), multivariate two-
sample tests.
23
...
1.0
p-value
(a) Randomly shufﬂed dataset with
...
1.00
Accuracy
(c) Randomly shufﬂed dataset with
same split proportions as original
dataset.
101
102
...
(g) Top different samples.
(h) Top similar samples.
Figure 43: CIFAR-10 medium image shift (50%, ﬁxed) plus knock-out shift (variable), univariate
...
BBSDs
(c) Knock out 100% of class 0.
Figure 44: CIFAR-10 medium image shift (50%, ﬁxed) plus knock-out shift (variable), multivariate
two-sample tests.
33
...
(g) Top different samples.
(h) Top similar samples.
Figure 45: CIFAR-10 only-zero shift (ﬁxed) plus medium image shift (variable), univariate two-
sample tests + Bonferroni aggregation.
101
102
103
...
BBSDs
(c) 100% perturbed samples.
Figure 46: CIFAR-10 only-zero shift (ﬁxed) plus medium image shift (variable), multivariate two-
sample tests.
34
