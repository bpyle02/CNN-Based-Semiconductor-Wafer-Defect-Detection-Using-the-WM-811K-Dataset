...
u
, ∀u ∈N(v)}, into a single vector hk−1
...
hk−1
v
, with the aggregated neighborhood vector, hk−1
...
representation hk−1
v
with the aggregated neighborhood vector hk
...
such that ∥xv −xv′∥2 > C for all pairs of nodes. Then we have that ∀ϵ > 0 there exists a parameter
...
Reddit data
To sample communities, we ranked communities by their total number of comments in
...
November, 2014.
Details on the source of the Reddit data are at:
...
WoS data
We selected the following subﬁelds manually, based on them being of relatively equal
...
PPI Tissue Data
For training, we randomly selected 20 PPI networks that had at least 15,000 edges.
...
C
Details on the Experimental Setup and Hyperparameter Tuning
Random walks for the unsupervised objective
For all settings, we ran 50 random walks of length
...
For the feature only model and to make predictions on the embeddings
...
Hardware
Except for DeepWalk, we ran experiments single a machine with 4 NVIDIA Titan X
...
Existing DeepWalk implementations [28, 11] are simply
...
Due to the heavy-tailed nature of degree distributions we
...
D
Alignment Issues and Orthogonal Invariance for DeepWalk and Related
Approaches
...
approximating any Hausdorff continuous, symmetric function to an arbitrary ϵ precision [29].
...
g(x) > ϵ,
if ∥x −xv∥2 = 0 for some v ∈D
g(x) ≤−ϵ,
if ∥x −xv∥2 > C, ∀v ∈D,
(7)
where ϵ < 0.5 is a chosen error tolerance.
Proof. Many such functions exist. For concreteness, we provide one construction that satisﬁes these
criteria. Let x ∈U denote an arbitrary input to g, let dv = ∥x −xv∥2, ∀v ∈D, and let g be deﬁned
as g(x) = P
v∈D gv(x) with
gv(x) =
3|D|ϵ
...
C2
> 0. By construction:
1. gv has a unique maximum of 3|D|ϵ −2ϵ > 2|D|ϵ at dv = 0.
16
...
g(x) = P
v∈D gv(a) ≤−ϵ by property 3 above. And, if dv = 0 for any v ∈D, then g is positive by
...
MLP and θσ its parameters, we have that ∀ϵ, ∃θσ such that |fθσ(x) −g(x)| < ϵ|, ∀x ∈U.
...
node, v, and deﬁne χ(G4) as the chromatic number of the graph with adjacency matrix A4 (ignoring
self-loops). Suppose that there exists a ﬁxed positive constant C ∈R+ such that ∥xv −xv′∥2 > C
...
h1
v ̸= h1
v′, ∀(v, v′) ∈{(v, v′) : ∃u ∈V, v, v′ ∈N 2(u)}, h1
v, h1
v′ ∈Eχ(G4)
I
where Eχ(G4)
I
is the set of one-hot indicator vectors of dimension χ(G4).
Proof. By the deﬁnition of the chromatic number, we know that we can label every node in V using
χ(G4) unique colors, such that no two nodes that co-occur in any node’s 2-hop neighborhood are
assigned the same color. Thus, with exactly χ(G4) dimensions we can assign a unique one-hot
...
By Lemma 1 and 2 and the assumption that ∥xv −xv′∥2 > C for all pairs of nodes, we can choose
an ϵ < 0.5 and there exists a single-layer MLP, fθσ, such that for any subset of nodes D ⊆V:
fθσ(xv) > 0,
∀v ∈D
fθσ(xv) < 0,
∀v ∈V \ D.
...
Moreover, we can create χ(G4) such MLPs, where each MLP corresponds to a different color/subset;
...
to an arbitrary ϵ precision [29]. Note that we can always account for normalization constants (line
...
v ∈EI. Thus, at depth k = 2 in Algorithm 1, suppose that we sum the
unnormalized representations of the neighboring nodes. Then without loss of generality, we will have
that h2
v = h1
v ⊕Av where A is the adjacency matrix of the subgraph containing all nodes connected
...
v vectors (i.e., m ≡χ(G4) from Lemma 3) and using
...
Thus, we have that b⊤c is the number of edges in the subgraph containing only v and it’s immediate
neighbors and Pm
i=0 b[i] = dv. Finally we can compute
2(b⊤c −dv)
(dv)(dv −1) = 2|{ev,v′ : v, v′ ∈N(v), ev,v′ ∈E}|
(dv)(dv −1)
(11)
= cv,
(12)
and since this is a continuous function of h3
v, we can approximate it to an arbitrary ϵ precision with
