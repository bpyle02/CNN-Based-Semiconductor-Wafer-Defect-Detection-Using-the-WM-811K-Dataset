...
17
3
Improving Efﬁciency and Effectiveness
18
3.1
...
30
3.4
Adapting ML Workﬂows for Federated Learning
. . . . . . . . . . . . . . . . . . . . . . . . . . . .
30
3.4.1
...
computations use wi-ﬁor slower con-
nections.
Addressability
...
cluding federated analytics tasks like model evaluation and debugging. Improving these is the focus of
...
wi-ﬁconnection, and idle, in order to avoid impacting the user of the device.
...
Fully decentralized algorithms for
...
The theoretical analysis of schemes which perform several local update
...
Similarly to the cross-device FL setting, an important task for
...
Privacy
An important challenge in fully decentralized learning is to prevent any client from reconstructing
...
the same ﬁnal model nonetheless (i.e. the free-rider problem). Related objectives include how to divide
...
The discussion of actors and threat models in Section 4.1 is largely relevant also for
...
Several works have also studied cross-silo federated tensor factorization where mul-
...
We ﬁrst survey some common ways in which data tend to deviate
from being identically distributed, that is Pi ̸= Pj for different clients i and j. Rewriting Pi(x, y) as
...
Violations of independence are introduced any time the distribution Q changes
...
Dataset shift
Finally, we note that the temporal dependence of the distributions Q and P may introduce
...
K is smaller than T/M3 then the (optimal) statistical term is dominating the rate. However, for typical
...
bound 1/
√
TKM if the number of local updates K are smaller than T 1/3/M. This convergence guarantee
was further improved by Wang and Joshi [467] who removed the bounded gradient norm assumption and
showed that the number of local updates can be as large as T/M3. The analysis in [467] can also be applied
...
for PL functions, T 2/M local updates per round leads to a O(1/TKM) convergence.
...
result, the error convergence in terms of training loss versus the total number of sequential SGD steps TK is
...
√
TKN (under non-
convex objectives), the number of local updates K should be smaller than T 1/3/N, instead of T/N3 as in
...
BNCVX
Each client function has bounded nonconvexity with ∇2fi(x) ⪰−µI.
BLGV
...
The objective function of federated optimization has been
...
Momentum, variance-reduction, and adaptive learn-
...
Motivated by the limited resources of current devices in terms of compute, mem-
...
speciﬁc and depend on the entropy of the underlying distribution [140]. In other words, if the data is
...
Many algorithms used in federated
...
The existing literature in federated learning usually neglects the impact of wire-
...
tion 3.5), it remains an important and active area. In particular, fully automating the process of
...
easily implicate concrete instantiations of several parameters (ε, ε′, ε0, γ, σ, etc.) as well as concepts such
...
Secure Multi-Party Computation (MPC) is a subﬁeld of cryptography
...
Homomorphic encryption (HE) schemes allow certain mathematical opera-
...
Secure Multi-Party Computation
Two or more participants collaborate to simulate, though cryptog-
...
Trusted execution environments
Trusted execution environments (TEEs, also referred to as secure en-
...
While secure multi-party computation and trusted execution
...
Secure aggregation
Secure aggregation is a functionality for n clients and a server. It enables each
...
in [308].
Secure shufﬂing
Secure shufﬂing is a functionality for n clients and a server. It enables each client
...
gorithm A is (ε, δ)-differentially private if for all S ⊆Range(A), and for all adjacent datasets D and D′:
P(A(D) ∈S) ≤eεP(A(D′) ∈S) + δ.
(3)
In the context of FL, D and D′ correspond to decentralized datasets that are adjacent if D′ can be obtained
...
jacency where D and D′ differ by only one record [165], since in general one user may contribute many
...
Differential privacy can be achieved without requiring trust in a centralized
...
with the guarantee holding with respect to any possible other local dataset D′. This model is referred to as the
...
One promising tool for achieving distributed DP in FL is secure
...
Distributed DP via secure shufﬂing
Another distributed differential privacy model is the shufﬂing
...
Another promising approach is hybrid differential privacy [40], which com-
...
Zero knowledge (ZK) proofs are a cryptographic primitive that enables
...
We discussed TEEs in Section 4.2.1, but focus
...
With this technique, the server clips the ℓ2 norm of individual updates, aggregates the clipped updates, and
...
Privacy ampliﬁcation for non-uniform device sampling procedures
Providing formal (ε, δ) guarantees
...
Most computational devices have access only to few
...
Privacy and security protocols are notoriously difﬁcult
...
well would a model that was learned privately with FL on a dataset D extend to a dataset D′ that was
...
to perform well on a new database D′. This is also related to questions of robustness that are explored
in Section 5.
...
Local differential privacy
As previously discussed, LDP removes the need for a trusted central server
...
Hybrid differential privacy
The hybrid model for differential privacy can help reduce the size of the
...
--- Page 55 ---
The shufﬂe model
The shufﬂe model enables users’ locally-added noise to be ampliﬁed through a shuf-
...
locally.
Secure aggregation
The Secure Aggregation protocols from [80, 58] have strong privacy guarantees when
...
Trade-offs for secure shufﬂing
These trade-offs have been recently studied in the shufﬂed model for
...
with (ε, δ)-differential privacy. The number of parties is n, and ℓis an integer parameter. Message sizes are
...
parameters ε and δ, and ˜Θ/ ˜O/˜Ωasymptotic notation suppresses factors that are polylogarithmic in B and
...
formally, we would like to solve the optimization problem ming,A ∥g(P
i A(xi)) −P
i xi∥, where ∥.∥can
be either the ℓ1 or ℓ2 norm. Of course, without enforcing any constraints on g(.) and A(·), we can always
choose them to be the identity function and get 0 error. However, A(·) has to satisfy two constraints: (1) A(·)
56

--- Page 57 ---
should output B bits (which can be thought of as the communication cost per user), and (2) ¯y = P
i A(xi)
should be an (ε, δ)-DP version of ¯x = P
...
budget. Looking at the problem differently, for a ﬁxed n, B, ε, and δ, what is the smallest ℓ1 or ℓ2 error that
...
fundamental interest to derive lower bounds and matching upper bounds on the ℓ1 or ℓ2 error under the
above constraints.
Privacy accounting
In the central model of DP, the subsampled Gaussian mechanism is often used to
...
The above model of distributed DP assumes that participating clients remain
...
New trust models
The federated learning framework motivates the development of new, more reﬁned trust
...
Goals
At a high level, adversarial attacks on machine learning models attempt to modify the behavior of
...
Capabilities
At the same time, an adversary may have a variety of different capabilities when trying to
...
Of particular importance to untargeted model update poisoning at-
...
One popular defense mechanism against untargeted model update poisoning
...
Targeted model update poisoning attacks may require fewer adversaries
...
Collusion defenses
Model update poisoning attacks may drastically increase in effectiveness if the adver-
...
Since data poisoning attacks induce model update
...
Defenses designed speciﬁcally for data poisoning attacks fre-
...
Since data poisoning attacks even-
...
The aforementioned discussion of eva-
...
The service provider can bound the contribution of
...
(such as the size of ℓ2 norm bounds and noise variance) can be chosen as a function of the model size and
architecture, as well as the fraction of malicious devices.
70

--- Page 71 ---
Defending against data poisoning attacks
Data poisoning can be thought of as a failure of a learning
...
Recall that in federated learning, each training round involves broadcasting a
...
While data pipelines in federated learning only exist within each client, there are
...
Another signiﬁcant complication is the limited ability to monitor devices
...
for a device i, and a ﬂeet-wide availability proﬁle A(t) = P
...
1. Rounds are started when at least M′ =
M
...
(b) The number of simultaneously connected devices: < M′ and a round cannot be started; ≫M′
...
device’s past contribution proﬁle (e.g. the number of rounds started or completed thus far). As
...
number of devices required to start a round, M′.
...
speciﬁc points in time to reach the threshold M′. Developing such a mechanism is difﬁcult due to
...
2042445.2042457.
[9] Naman Agarwal, Ananda Theertha Suresh, Felix X. Yu, Sanjiv Kumar, and Brendan McMahan.
cpSGD:
...
[30] Anish Athalye, Nicholas Carlini, and David Wagner.
...
URL https://content.sciendo.com/view/journals/popets/
2020/4/article-p48.xml.
[40] Brendan Avent, Aleksandra Korolova, David Zeber, Torgeir Hovden, and Benjamin Livshits.
BLENDER:
...
[45] Borja Balle, James Bell, Adri`a Gasc´on, and Kobbi Nissim. The privacy blanket of the shufﬂe model. In
...
--- Page 93 ---
[53] Raef Bassily and Adam Smith. Local, private, efﬁcient protocols for succinct histograms. In STOC, pages
127–135, 2015.
...
[73] Andrea Bittau, ´Ulfar Erlingsson, Petros Maniatis, Ilya Mironov, Ananth Raghunathan, David Lie, Mitch
...
[111] Lingjiao Chen, Hongyi Wang, Zachary B. Charles, and Dimitris S. Papailiopoulos.
DRACO: Byzantine-
resilient distributed training via redundant gradients.
In Proceedings of the 35th International Conference
on Machine Learning, ICML, 2018.
[112] Mingqing Chen, Rajiv Mathews, Tom Ouyang, and Franc¸oise Beaufays.
Federated learning of out-of-
...
--- Page 98 ---
[137] Andrew Cotter, Ohad Shamir, Nati Srebro, and Karthik Sridharan.
Better mini-batch algorithms via ac-
...
[155] Bolin Ding, Janardhan Kulkarni, and Sergey Yekhanin.
...
[177] ´Ulfar Erlingsson, Vasyl Pihur, and Aleksandra Korolova.
RAPPOR: Randomized aggregatable privacy-
...
[178] ´Ulfar Erlingsson, Vitaly Feldman, Ilya Mironov, Ananth Raghunathan, Kunal Talwar, and Abhradeep Thakurta.
...
[183] Minghong Fang, Xiaoyu Cao, Jinyuan Jia, and Neil Zhenqiang Gong.
...
html.
[207] Antonious M Girgis, Deepesh Data, Suhas Diggavi, Peter Kairouz, and Ananda Theertha Suresh.
Shuf-
ﬂed model of federated learning:
Privacy, communication and accuracy trade-offs.
arXiv preprint
arXiv:2008.07180, 2020.
[208] O. Goldreich, S. Micali, and A. Wigderson. How to play any mental game. In Proceedings of the Nine-
...
28395.28420.
[209] ShaﬁGoldwasser, Silvio Micali, and Charles Rackoff. The knowledge complexity of interactive proof systems.
SIAM J. Comput., 18(1):186–208, 1989.
[210] ShaﬁGoldwasser, Yael Tauman Kalai, and Guy N. Rothblum. Delegating computation: interactive proofs for
...
[232] ´Ursula H´ebert-Johnson, Michael Kim, Omer Reingold, and Guy Rothblum. Multicalibration: Calibration for
the (computationally-identiﬁable) masses. In International Conference on Machine Learning, pages 1944–
1953, 2018.
...
defenses. In International Joint Conference on Artiﬁcial Intelligence (IJCAI), Macao, China, 2019. URL
...
without centralized training data, April 2017.
...
--- Page 110 ---
[347] Ilya Mironov, Omkant Pandey, Omer Reingold, and Salil Vadhan.
...
[349] Shira Mitchell, Eric Potash, and Solon Barocas.
...
[359] Valeria Nikolaenko, Udi Weinsberg, Stratis Ioannidis, Marc Joye, Dan Boneh, and Nina Taft.
Privacy-
...
[373] Giorgio Patrini, Richard Nock, Stephen Hardy, and Tib´erio S. Caetano.
Fast learning from distributed
datasets without entity matching.
In Proceedings of the Twenty-Fifth International Joint Conference on
Artiﬁcial Intelligence, IJCAI 2016, New York, NY, USA, 9-15 July 2016, pages 1909–1917, 2016.
URL
...
2020.
URL
https://ai.googleblog.com/2020/05/
...
rera. Federated learning and differential privacy: Software tools analysis, the sherpa. ai ﬂframework and
...
[429] Liwei Song, Reza Shokri, and Prateek Mittal.
...
[432] Jacob Steinhardt, Pang Wei W Koh, and Percy S Liang. Certiﬁed defenses for data poisoning attacks. In
...
data. In ICML, 2018.
[444] Hanlin Tang, Xiangru Lian, Shuang Qiu, Lei Yuan, Ce Zhang, Tong Zhang, and Ji Liu.
DeepSqueeze:
...
--- Page 117 ---
[468] Jianyu Wang and Gauri Joshi.
Adaptive Communication Strategies for Best Error-Runtime Trade-offs in
Communication-Efﬁcient Distributed SGD.
In Proceedings of the SysML Conference, April 2019.
URL
https://arxiv.org/abs/1810.08313.
[469] Jianyu Wang, Anit Sahu, Gauri Joshi, and Soummya Kar.
MATCHA: Speeding Up Decentralized SGD
via Matching Decomposition Sampling. preprint, May 2019. URL https://arxiv.org/abs/1905.
09435.
[470] Jianyu Wang, Vinayak Tantia, Nicolas Ballas, and Michael Rabbat.
SlowMo: Improving communication-
...
communication-efﬁcient local updates. In European Conference on Machine Learning and Principles and
...
Simulations of federated learning require dealing with multiple issues that do not
...
Datasets
Federated learning is adopted when the data is decentralized and typically unbalanced (different
...
FL problem.
Cross-silo datasets
One example is the iNaturalist dataset13 which consists of large numbers of observa-
...
The need for more datasets
Developing new federated learning datasets that are representative of real-
