...
2N ||θ||2
(1)
with N data points, dropout probability p, samples c
Wi ∼q∗
...
Wt(x))
(3)
with T sampled masked model weights c
Wt ∼q∗
...
PT
t=1 f c
Wt(x). The ﬁrst term in the predictive variance, σ2, corresponds to the amount of noise
...
when we have zero parameter uncertainty (i.e. when all draws c
Wt take the same constant value).
2.2
...
--- Page 5 ---
we draw model weights from the approximate posterior c
W ∼q(W) to obtain a model output, this
time composed of both predictive mean as well as predictive variance:
[ˆy, ˆσ2] = f
c
W(x)
(6)
where f is a Bayesian convolutional neural network parametrised by model weights c
W. We can use
...
t = f c
Wt(x) for randomly masked weights
c
Wt ∼q(W).
3.2
...
3.3
Heteroscedastic Uncertainty in Classiﬁcation Tasks
...
[20] Simon J´egou, Michal Drozdzal, David Vazquez, Adriana Romero, and Yoshua Bengio.
The one
...
2012.
[24] Liang-Chieh Chen, George Papandreou, Iasonas Kokkinos, Kevin Murphy, and Alan L Yuille.
Se-
...
2014.
[28] Vijay Badrinarayanan, Alex Kendall, and Roberto Cipolla.
