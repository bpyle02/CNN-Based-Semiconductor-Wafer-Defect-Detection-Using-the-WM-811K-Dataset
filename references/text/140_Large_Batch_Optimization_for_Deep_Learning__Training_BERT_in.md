...
**Authors**: You, Li, Xu, He, Ginsburg, Hsieh
**Year**: 2020
**arXiv**: 1904.00962
**Topic**: optimization
...
optimizers/lamb.py
1
arXiv:1904.00962v5  [cs.LG]  3 Jan 2020
...
PRELIMINARIES
Notation.
For any vector xt ∈Rd, either xt,j or [xt]j are used to denote its jth coordinate where
...
elementwise square and division operators respectively. We use ∥.∥and ∥.∥1 to denote l2-norm and
...
min
x∈Rd f(x) := Es∼P[ℓ(x, s)] + λ
2 ∥x∥2,
(1)
where ℓis a smooth (possibly nonconvex) function and P is a probability distribution on the domain
S ⊂Rk. Here, x corresponds to model parameters, ℓis the loss function and P is an unknown data
distribution.
We assume function ℓ(x) is Li-smooth with respect to x(i), i.e., there exists a constant Li such that
∥∇iℓ(x, s) −∇iℓ(y, s)∥≤Li∥x(i) −y(i)∥,
∀x, y ∈Rd, and s ∈S,
(2)
for all i ∈[h]. We use L = (L1, · · · , Lh)⊤to denote the h-dimensional vector of Lipschitz constants.
We use L∞and Lavg to denote maxi Li and P
i
Li
h respectively. We assume the following bound
on the variance in stochastic gradients: E∥∇iℓ(x, s) −∇if(x)∥2 ≤σ2
i for all x ∈Rd and i ∈[h].
Furthermore, we also assume E∥[∇ℓ(x, s)]i −[∇f(x)]i∥2 ≤˜σ2
i for all x ∈Rd and i ∈[d]. We use
σ = (σ1, · · · , σh)⊤and ˜σ = (˜σ1, · · · , ˜σd)⊤to denote the vectors of standard deviations of stochastic
...
2. The learning rate is scaled by φ(∥xt∥) for some function φ : R+ →R+. Similar to the
...
Input: x1 ∈Rd, learning rate {ηt}T
t=1, parameters
0 < β1, β2 < 1, scaling function φ, ϵ > 0
Set m0 = 0, v0 = 0
for t = 1 to T do
Draw b samples St from P.
Compute gt =
...
two ways: (1) the convergence criterion is (E[Ph
...
To ensure that our approach is compared to a solid
...
t ∥
!
1(sign(∇if(xt)]j) ̸= sign(r(i)
t,j))
(8)
This follows from the fact that ∥r(i)
t ∥≤
q
di
1−β2 and √vt ≤G. If β2 = 0, then T1 can be bounded
as follows:
T1 ≤−ηt
h
X
...
E
h
αu|[∇if(xt)]j|1(sign(∇if(xt)]j) ̸= sign(g(i)
t,j))
i
≤−ηt
h
...
X
j=1
αu|[∇if(xt)]j|P(sign(∇if(xt)]j) ̸= sign(g(i)
t,j))
(9)
...
Input: x1 ∈Rd, learning rate {ηt}T
t=1, parame-
ters 0 < β1, β2 < 1, scaling function φ, ϵ > 0,
parameters 0 < {βt
1}T
t=1 < 1
Set m0 = 0, v0 = 0
...
Input: x1 ∈Rd, learning rate {ηt}T
t=1, parameters
0 < β1, β2 < 1, scaling function φ, ϵ > 0, parame-
ters 0 < {βt
1}T
t=1 < 1
Set m0 = 0, v0 = 0
...
ϵ = 1e-8. We used the same settings for N-LAMB and NN-LAMB. We scaled the batch size to 32K
...
default setting for other parameters: weight decay rate λ=0.01, β1=0.9, β2=0.999, ϵ=1e-6. We did not
