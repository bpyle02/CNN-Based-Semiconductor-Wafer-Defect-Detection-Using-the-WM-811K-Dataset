...
class Krum:
    ...
    def aggregate(
        ...
        for update_dict in client_updates:
            flat_vec = torch.cat([v.flatten() for v in update_dict.values()])
            flat_updates.append(flat_vec)

        flat_updates_tensor = torch.stack(flat_updates)  # (n_clients, n_params)

        # Compute pairwise distances
        distances = torch.cdist(flat_updates_tensor, flat_updates_tensor)  # (n_clients, n_clients)
...
class MultiKrum:
    ...
    def aggregate(
        ...
        for _ in range(min(num_selected, len(client_updates))):
            ...
            remaining_updates = [u for u in remaining_updates if u is not selected]
