...
def load_data_splits(
    ...
    X_pool, X_test, y_pool, y_test = train_test_split(
        np.arange(len(labels)), labels, test_size=0.20, stratify=labels, random_state=seed
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--model", choices=["cnn", "resnet", "effnet", "all"], default="cnn", help="Model to use"
    )
    parser.add_argument(
        "--initial-labeled", type=float, default=0.1, help="Fraction of pool to start with (0-1)"
    )
    parser.add_argument(
        "--acquisition-size", type=int, default=500, help="Samples to label per iteration"
    )
    parser.add_argument(
        "--n-iterations", type=int, default=3, help="Number of active learning iterations"
    )
    parser.add_argument(
        "--strategy",
        choices=["entropy", "margin", "least_confidence"],
        default="entropy",
        help="Uncertainty strategy",
    )
    parser.add_argument(
        "--prioritized",
        action="store_true",
    ...
    if args.prioritized:
        ...
        logger.info(
            f"Prioritized sampling: alpha={args.alpha}, " f"beta={args.beta_start}->{args.beta_end}"
        )
    ...
    pool_maps, y_pool, test_maps, y_test, class_names = load_data_splits(
    ...
    for model_type in models_to_train:
        ...
        for iteration in range(args.n_iterations):
            ...
            unlabeled_dataset = WaferMapDataset(
                pool_maps[current_unlabeled],
                y_pool[current_unlabeled],
                transform=None if model_type == "cnn" else imagenet_norm,
            )
            ...
            if selector is not None:
                ...
                logger.info(
                    f"    Priority stats: mean={stats['priority_mean']:.4f}, "
                    f"max={stats['priority_max']:.4f}, min={stats['priority_min']:.4f}, "
                    f"std={stats['priority_std']:.4f}"
                )
                logger.info(
                    f"    IS weights: mean={stats['weight_mean']:.4f}, "
        ...
        for res in iteration_results:
            logger.info(
                f"    Iter {res['iteration']}: Labeled={res['labeled']:,}, Accuracy={res['accuracy']:.4f}, F1={res['f1']:.4f}"
            )
