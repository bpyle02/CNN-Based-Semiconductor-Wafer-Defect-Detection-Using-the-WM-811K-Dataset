...
def test_krum_aggregation_resists_outlier():
    """Test Krum prefers the honest cluster over an extreme outlier."""
    logger.info("Test 5b: Krum robust aggregation")
    ...
    aggregated = aggregator.aggregate(weights)

    assert aggregated['w'].item() != 100.0, "Krum should reject the outlier update"
    assert abs(aggregated['w'].item()) < 0.1, "Krum should select an honest client update"
    logger.info("  PASS: Krum selects an honest update")
...
def test_checkpoint_save_load():
    ...
    checkpoint_dir = Path.cwd() / "tests" / ".tmp_checkpoints"
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = checkpoint_dir / "test_ckpt.pth"
...
def main():
    ...
    try:
        ...
        test_krum_aggregation_resists_outlier()
