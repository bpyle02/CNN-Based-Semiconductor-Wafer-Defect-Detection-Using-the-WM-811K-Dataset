...
def load_and_split_data(
    ...
    val_size: float = 0.15,
    seed: int = SEED,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, LabelEncoder]:
    ...
    return train_maps_raw, val_maps_raw, test_maps_raw, y_train, y_val, y_test, le
...
def main():
    ...
    train_maps_raw, val_maps_raw, test_maps_raw, y_train, y_val, y_test, le = load_and_split_data(
        args.data_path, test_size=config.data.test_size, val_size=config.data.val_size,
        seed=config.seed
    )
    ...
    for model_type in models_to_train:
        ...
        for stage_idx, stage in enumerate(stages):
            ...
            train_loader, val_loader, test_loader = create_dataloaders(
                train_maps_raw, val_maps_raw, test_maps_raw,
            ...
            optimizer = optim.Adam(
                ...
                lr=stage_lr, weight_decay=config.training.weight_decay
            )
            ...
            model, history = train_model(
                ...
                scheduler=scheduler, epochs=epochs, model_name=f"{model_name} (Stage {stage_idx + 1})",
